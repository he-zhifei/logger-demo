...
public class Log4j2Test02 {
    ...
    /**
     * 测试自定义logger不会走混合异步的情况，因为配置的异步logger名为com.zhifei.log4j2，而这里实际的是com.zhifei.log4j2_2
     */
    @Test
    public void test03() {
        Logger logger = LoggerFactory.getLogger("com.zhifei.log4j2_2");
        for (int i = 0; i < 2000; i++) {
            printAllLevelLog(logger);
        }
        for (int i = 1; i <= 1000; i++) {
